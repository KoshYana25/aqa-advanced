...
describe("Registration Functionality Tests", () => {
  ...
  it("Should validate the password field", () => {
    cy.get("button").contains("Registration").click(); 
    cy.get('input[id="signupPassword"]').type("Short1", { sensitive: true }).blur();
    cy.get('input[id="signupPassword"]')
      .siblings(".invalid-feedback")
      .find("p")
      .should(
    ...
    cy.get('input[id="signupPassword"]').clear().type("lowercase1", { sensitive: true }).blur();
    cy.get('input[id="signupPassword"]')
      .siblings(".invalid-feedback")
      .find("p")
      .should(
  ...
  it("Should validate the re-enter password field", () => {
    cy.get("button").contains("Registration").click(); 
    cy.get('input[id="signupPassword"]').type("ValidPass1", { sensitive: true });
    cy.get('input[id="signupRepeatPassword"]').type("DifferentPass1", { sensitive: true }).blur();
    cy.get('input[id="signupRepeatPassword"]')
      .siblings(".invalid-feedback")
      .find("p")
  ...
  it("Should successfully register with valid data", () => {
    ...
    cy.get('input[id="signupEmail"]').type("yana277@gmail.com");
    cy.get('input[id="signupPassword"]').type("ValidPass1", { sensitive: true });
    cy.get('input[id="signupRepeatPassword"]').type("ValidPass1", { sensitive: true });
  ...
  it("Should successfully log in with valid credentials", () => {
    ...
    cy.get('input[id="signinPassword"]').type("ValidPass1", { sensitive: true });
